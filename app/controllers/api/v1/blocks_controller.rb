class Api::V1::BlocksController < ApplicationController
  ...
  def create
    ...
    current_user.blocks_given.create!(blocked: blocked_user)
    ...
    if everyone_group_other
        association = UsersContactGroup.find_by(user: current_user, contact_group: everyone_group_other)
    ...
    if everyone_group
        association = UsersContactGroup.find_by(user: blocked_user, contact_group: everyone_group)
        association.destroy if association
    end
    

    redirect_to chatroom_path(current_user), notice: "User has been blocked and the conversation was deleted."
  end
